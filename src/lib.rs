...
const _URL: &str = "https://mempool.space/api/blocks/tip/height";
...
//
//                let body = reqwest::get(_URL).await.unwrap().bytes();
...
//         .map(|_| {
//             std::thread::spawn(|| {
//                 let mut body = ureq::get(_URL).call().expect("REASON").into_reader();
//                 let mut buf = Vec::new();
