...
impl<'a> Uploader<'a> {
    ...
    fn upload(&self, mut multipart: Multipart<'static, '_>) -> Result<String> {
        ...
        let mut request = self.client.post(&self.config.server.address).set(
            "Content-Type",
            &format!("multipart/form-data; boundary={}", multipart_data.boundary()),
        );
        ...
        let result = match request.send(upload_tracker) {
            Ok(response) => {
                ...
                if response_text.lines().count() != 1 {
                    ...
                    Err(Error::UploadError(format!("unknown error (status code: {status})")))
                }
            }
        ...
        let result = match request.call() {
            Ok(response) => {
                ...
                if status == 200 {
                    ...
                    Err(Error::DeleteError(format!("unknown error (status code: {status})")))
                }
            }
            Err(UreqError::Status(code, response)) => {
                if code == 404 {
                    Err(Error::DeleteError(response.into_string()?.trim().to_string()))
                } else {
    ...
    pub fn retrieve_list<Output: Write>(&self, output: &mut Output, prettify: bool) -> Result<()> {
        ...
        let response = request.call().map_err(|e| Error::RequestError(Box::new(e)))?;
        if !prettify {
        ...
        let filename_width = items.iter().map(|v| v.file_name.len()).max().unwrap_or_default();
        ...
        items.iter().try_for_each(|file_info| {
            writeln!(
                ...
                file_info.expires_at_utc.as_ref().cloned().unwrap_or_default()
            )
        })?;
        Ok(())
    }
